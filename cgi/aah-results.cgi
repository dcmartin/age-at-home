...
if ($?QUERY_STRING) then
    ...
    set all_scores = `/bin/echo "$QUERY_STRING" | sed 's/.*all_scores\([^&]*\).*/\1/'`
...
if (! -s "$OUTPUT") then
  ...
  if ($?TEST) then
	...
	foreach m ( $results )
	  ...
	  if ($?all_scores) then
	    jq '.rows[]|select(.id=="'$m'")' "$TEST" | \
	      jq '{"model":.doc.model,"results":[.doc.sets[]|{"class":.set,"all":[.results[]|{"id":.id,"scores":.classes|sort_by(.score)}]}]}' >! "$OUTPUT.$$.$$"
	  else
	    jq '.rows[]|select(.id=="'$m'")' "$TEST" | \
	      jq '{"model":.doc.model,"results":[.doc.sets[]|{"class":.set,"top":[.results[]|{"id":.id,"class":.classes|sort_by(.score)[-1].class}]}]}' >! "$OUTPUT.$$.$$"
	  endif
	...
	goto output
      ...
      /bin/echo `date` "$0:t $$ -- no testing cache $TEST" >>&! $LOGTO
    endif
  else
    /bin/echo `date` "$0:t $$ -- no model data" >>&! $LOGTO
  endif
else
  /bin/echo `date` "$0:t $$ -- $OUTPUT is up-to-date ($DATE)" >>&! $LOGTO
endif

output:

if (! -s "$OUTPUT") then
  if ($?DEBUG) /bin/echo `date` "$0:t $$ -- no results found ($db)" >>&! $LOGTO
