...
/bin/echo -n `date` "$0:t $$ -- db=$DB id=$class " >>&! $LOGTO
if ($?day) then
...
if (-e "$JSON") then
    /bin/echo `date` "$0:t $$ == CURRENT $JSON $DATE" >>&! $LOGTO
else
    /bin/echo `date` "$0:t $$ ++ MAKING ($JSON)" >>&! $LOGTO
    ./$APP-make-$API.bash
    # find old results
	...
	set JSON = "$TMP/$APP-$API-$QUERY_STRING.$DATE.json"
        if (! -e "$JSON") then
	    /bin/echo `date` "$0:t $$ -- RETRIEVING ($JSON)" >>&! $LOGTO
	    curl -s -q -o "$JSON" "https://$CU/$DB-$API/$class"
	endif
	set ERROR = `jq '.error' "$JSON" | sed 's/"//g'`
	if ($ERROR == "not_found") then
	    /bin/echo `date` "$0:t $$ -- ERROR ($ERROR)" >>&! $LOGTO
            rm -f "$JSON"
	    exit
        endif
    else if ($#OLD_JSON > 0) then
	/bin/echo `date` "$0:t $$ == OLD $JSON ($OLD_JSON)" >>&! $LOGTO
	set JSON = "$OLD_JSON[1]"
	if ($#OLD_JSON > 1) then
	    /bin/echo `date` "$0:t $$ -- DELETING $OLD_JSON[2-]" >>&! $LOGTO
	    /bin/rm -f "$OLD_JSON[2-]"
	endif
    endif
endif
...
if ($#JSON == 0) then
    /bin/echo `date` "$0:t $$ ** NONE ($JSON)" >>&! $LOGTO
    /bin/echo "Status: 202 Accepted"
    goto done
else if (-e "$JSON") then
    /bin/echo `date` "$0:t $$ --" `ls -al $JSON` >>&! $LOGTO
endif
...
if ($?day && $?interval) then
    if ($day == "all" && $interval == "all") then
        /bin/echo `date` "$0:t $$ -- CALCULATING day ($day) and interval ($interval)" >>&! $LOGTO
...
if ($?day) then
    if ($day != "all") then
        /bin/echo -n `date` "$0:t $$ -- CALCULATING day ($day) " >>&! $LOGTO
...
# test interval
    ...
    if ($interval != "all") then
        /bin/echo `date` "$0:t $$ -- CALCULATING interval ($interval)" >>&! $LOGTO
