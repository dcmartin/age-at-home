...
if (! -e "$OUTPUT") then
    ...
    if ($DB == "damp-cloud") then
	cat "$OUTPUT" \
	    | sed "s/Dampcloud 15 Minute Interval/Interval/" \
	    | sed "s/\([^ ]\) Dampcloud Visual Scores Score Avg/\1/g" >> "$OUTPUT".$$
    else
	cat "$OUTPUT" \
	    | sed "s/Roughfog 15 Minute Interval/Interval/" \
	    | sed "s/\([^ ]*\) Roughfog Visual Scores Score Avg/\1/g" >> "$OUTPUT".$$
    endif
    mv -f "$OUTPUT".$$ "$OUTPUT"
endif
